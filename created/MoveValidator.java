...
// Not sure if code below accounts for hits, added isActive to piece class
            ...
            Piece king = board.getTeam(piece.getPieceColor()).stream().filter(p -> p.pieceType == ChessPiece.KING).findFirst().get();
...
//            if (board.getTeam(piece.getPieceColor()).stream()
//                    .noneMatch(p -> p.listOfMoveMores.stream()
//                            .filter(m -> m.getTo().equals(king.getPiecePosition()))
//                            .noneMatch(m -> isMovePossibleNoSelfCheck(m, board)))) {
//                isMovePossible = false;
//            }
...
//         is destination under attack and piece moving is king?
    ...
    public boolean isMovePossibleNoSelfCheck(MoveMore move, Board board) {
        if (board.getPieceAtPosition(move.getFrom()) == null)
            System.out.println("No piece at position " + move.getFrom());
        Piece piece = board.getPieceAtPosition(move.getFrom());
        Color oppositeColor = board.getPieceAtPosition(move.getFrom()).getPieceColor() == Color.WHITE ? Color.BLACK : Color.WHITE;
        ...
        boolean isMovePossible = true;
        // is destination field occupied by enemy piece and move is not pawn move only?
        if (board.getPieceAtPosition(move.getTo()) != null && board.getPieceAtPosition(move.getTo()).pieceColor != piece.getPieceColor() && piece.pieceType == ChessPiece.PAWN && move.isHit() != true)
            isMovePossible = false;
        // is destination empty and move is pawn attack?
        if (board.getPieceAtPosition(move.getTo()) == null  && piece.pieceType == ChessPiece.PAWN && move.isHit() == true)
            isMovePossible = false;
